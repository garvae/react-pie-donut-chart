...
describe('prop "data"', () => {
  it('generates a segment when only one element in the "data" array', () => {
    ...
    const { container, rerender } = render(<PieDonutChart {...TEST_CHART_PROPS_COMMON} />);

    expect(container.querySelectorAll('path')).toHaveLength(TEST_PROPS.data.length);

    rerender(<PieDonutChart data={FAKE_DATA_SINGLE_SEGMENT} size={TEST_SIZE_CHART} />);
  ...
  it('generates a number of segments equal to the number of elements of the "data" array', () => {
    ...
    rerender(<PieDonutChart data={FAKE_DATA_SLICED} size={TEST_SIZE_CHART} />);
  ...
  it('generates a chart when only one element in the "data" array have value > 0', () => {
    ...
    const { container } = render(
      <PieDonutChart {...TEST_CHART_PROPS_COMMON} data={FAKE_DATA_ONLY_ONE_VALID_SEGMENT} />
    );
  ...
  it('shows an error when the provided required prop "data" is invalid', () => {
    ...
    expect(multilineToSingleLineLowerCased(consoleCallText)).toContain(
      multilineToSingleLineLowerCased(INVALID_DATA_DEFAULT_ERROR)
    );
  });
});
