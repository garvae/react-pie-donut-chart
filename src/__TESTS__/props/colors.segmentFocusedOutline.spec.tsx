...
import { TEST_DATA_ATTR_CHART_GROUP_SEGMENT_ID, TEST_DATA_ID_CHART_GROUP_SEGMENT } from 'components/Chart';
...
describe('prop "colors.segmentFocusedOutline"', () => {
  it('sets the provided color for the "outline" of the segment', () => {
    ...
    const { getAllByTestId } = render(
      <PieDonutChart {...TEST_CHART_PROPS_COMMON} colors={{ segmentFocusedOutline }} selected={selectedSegmentId} />
    );

    const segmentToFocus = getAllByTestId(TEST_DATA_ID_CHART_GROUP_SEGMENT).find(
      (item) => item.getAttribute(TEST_DATA_ATTR_CHART_GROUP_SEGMENT_ID) === selectedSegmentId
    );
