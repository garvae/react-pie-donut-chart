...
describe('prop "animationSpeed"', () => {
  it('is applied to segments', async () => {
    ...
    const { getAllByTestId } = render(<PieDonutChart {...TEST_CHART_PROPS_COMMON} animationSpeed={animationSpeed} />);

    await waitFor(
      () => {
        getAllByTestId(TEST_DATA_ID_CHART_GROUP_SEGMENT).forEach((item) =>
          expect(item).toHaveStyle(`transition-duration: ${animationSpeed}ms`)
        );
      },
